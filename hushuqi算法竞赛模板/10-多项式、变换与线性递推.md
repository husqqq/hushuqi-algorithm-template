...
以下前导在本章开头只印一次。后续 C++ 代码块均与此前导拼接使用，不再重复头文件、整数宏、公共常量和 `chmin` / `chmax`。模整数显式依赖 [7.1.007 静态取模类](07-数论.md#71007) 的 `MInt`、`invTable`、`mod` 与 `Z`；该实现已经完整打印，本章不再复制。
...
本章固定使用 `Z=MInt<998244353>`；性能版为 4 字节代表元，打印版按 7.1.007 使用 `int` 代表元（在公共 `#define int long long` 下通常为 8 字节）。NTT 和在线卷积只依赖 `Z` 的模运算接口，不依赖这两个版本的存储宽度。运行时素数模的有限域分解另有明确的动态模专用实现，不与固定模 `Z` 混用。
...
`fwt(a,op,inv)` 的 `op` 取 `BitOp::Or`、`BitOp::And` 或 `BitOp::Xor`；正变换后逐点相乘，再用同一枚举值逆变换。异或逆变换要求长度在系数域中可逆。`orFWT<Inv>(a,n,d)` 要求 $n$ 为二次幂，连续区间恰有 $nd$ 项；时间 $O(nd\log n)$、额外空间 $O(1)$。`bitConv` 封装通用流程。固定模热点调用 `fwtMod<P,Op,Inv>(a)` 或 `bitConvMod<P,Op>(a,b)`，输入必须已经落在 $[0,P)$；$P<2^{31}$，异或还要求 $P$ 为奇数。本机长度 $2^{20}$ 的 AND 正逆变换中，固定模版 7 轮中位数约为 `4ms`，通用 `MInt` 版约为 `62ms`；代价是代码体积更大且不支持运行时模数。两版渐进复杂度均为 $O(N\log N)$。$2$ 在模下不可逆时改用 [10.1.007 偶模 FWT 与光速乘](#101007)，每位取值超过 $2$ 种用 [10.1.008 K 进制 FWT](#101008)，要求不交拆分时用 [10.1.011 子集卷积](#101011)。
...
把每个 `uint64_t` 视为 $GF(2)[x]$ 中次数小于 $64$ 的多项式，并模不可约多项式 $x^{64}+x^4+x^3+x+1$，即可得到 $GF(2^{64})$。加减都是异或；`carrylessMul` 做无进位乘法，`GF64::reduce` 用多项式 Montgomery 约简。乘法群阶 $2^{64}-1$ 没有足够大的二次幂因子，不能套普通 NTT；下面用特征 $2$ 的 additive FFT，在一组 $GF(2)$ 线性无关的基上递归求值。x86-64 的 GCC/Clang 路径使用 PCLMUL 完成常数时间无进位乘法；其他目标保留逐置位的可移植回退。
...
struct GF64
{
    ...
    static uint64_t reduce(unsigned __int128 x)
    {
        ...
        return (uint64_t)(x >> 64) ^
               (uint64_t)(carrylessMul(f, (uint64_t)poly) >> 64) ^ f;
    }
...
`convGF64(a,b)` 返回上述固定域中的线性卷积，空输入返回空；输入和输出都是普通多项式基下的 `uint64_t`。设补齐长度为 $N$，每层 additive FFT 的 Taylor 变换还要递归处理半长块，当前实现时间为 $O(N\log^2 N)$ 次域运算、空间为 $O(N\log N)$；当结果长度恰为 $2^k+1$ 时会拆下较长输入的末项，避免补齐长度翻倍。PCLMUL 是本题最大规模下的重要常数优化，非 x86-64 平台的标量回退仍保持相同复杂度但可能无法满足严格时限。模 `998244353` 的整数卷积仍用 [10.1.003 NTT](#101003)。
...
形式导数把 $a_ix^i$ 变为 $ia_ix^{i-1}$，积分做逆操作 $a_ix^i\mapsto\frac{a_i}{i+1}x^{i+1}$ 并把常数项固定为零，要求 $1..n$ 在模下可逆。积分直接复用 [7.1.007 静态取模类](07-数论.md#71007) 的共享 `invTable<mod>`，表只在首次需要更大下标时线性扩展，不逐项调用费马逆元。`batchInv` 用一组前缀积、一次总逆元和一次反向扫描求数组逐项逆元；零项不进入乘积并在答案中保持零。三者是 FPS、插值和点值平移共用的基础零件。
...
FPS 只有少量非零项时，逆、对数、指数、幂和平方根都能按非零项递推，复杂度依赖有效非零项数 $k$ 而非 $n^2$。前置 [10.2.006 稀疏多项式除法](#102006) 的 `spNorm` 负责排序、合并同次项和删除零项；各递推共同复用 [7.1.007 静态取模类](07-数论.md#71007) 的 `invTable<mod>`，不再维护另一份稀疏专用逆元表。exp 由 $g'=gf'$，log 由 $(\log f)'=f'/f$，pow 由 $g'f=kgf'$；把最低项写成 $f=x^pcu$ 且 $u(0)=1$ 后，幂先平移 $pk$ 次，平方根要求 $p$ 为偶数并对单位部分使用指数 $\frac12$ 的同类递推。
