...
struct Cover2Seg
{
    ...
    void add(int l, int r, int v)
    {
        ...
        if (l < r) add(1, 0, (int)ys.size() - 1, l, r, v);
    }
...
struct StaticMex
{
    ...
    StaticMex(const vector<int> &a) : n(a.size()), tr(1, Node{0, 0, -1}), rt(n + 1)
    {
        ...
        {
            rt[i + 1] = rt[i];
            if (0 <= a[i] && a[i] <= n) rt[i + 1] = set(rt[i], 0, n + 1, a[i], i);
        }
    }
    ...
    int set(int p, int l, int r, int x, int v)
    {
        ...
        auto m = (l + r) / 2;
        if (x < m) tr[q][0] = set(tr[q][0], l, m, x, v);
        else tr[q][1] = set(tr[q][1], m, r, x, v);
    ...
    int find(int p, int l, int r, int lim) const
    {
        ...
        if (r - l == 1) return l;
        auto m = (l + r) / 2;
        if (val(tr[p][0]) < lim) return find(tr[p][0], l, m, lim);
...
struct ISAP
{
    ...
    int dfs(int x, int t, int lim)
    {
        ...
        {
            auto &a = e[x][i];
            if (a[2] == 0 || h[x] != h[a[0]] + 1) continue;
            auto d = dfs(a[0], t, min(lim - used, a[2]));
            a[2] -= d;
            e[a[0]][a[1]][2] += d;
            used += d;
            if (used == lim || h[x] == n) return used;
        }
        if (--gap[h[x]] == 0) h[src] = n;
        auto nh = n - 1;
        for (auto a : e[x])
        {
            if (a[2]) nh = min(nh, h[a[0]]);
        }
        h[x] = nh + 1;
        gap[h[x]]++;
        cur[x] = 0;
...
int minModInt(int m, int d, int l, int r)
{
    ...
    if (l == 0) return 0;
    if (d == 0) return -1;
    if (2 * d > m) return minModInt(m, m - d, m - r, m - l);
...
pair<int, int> goldbach(int n)
{
    ...
    if (n < 4 || n % 2) return {-1, -1};
    auto is = primeTable(n);
    for (int p = 2; p <= n; p++)
    {
        if (is[p] && is[n - p]) return {p, n - p};
    }
    return {-1, -1};
}
...
int rectKind(const array<GeoPoint, 4> &p)
{
    ...
    if (d[0] <= 1E-24L || !eq(d[0], d[1]) || !eq(d[2], d[3]) || !eq(d[4], d[5]) || !eq(d[0] + d[2], d[4])) return 0;
...
bool boundSubset(const vector<int> &cnt, const vector<int> &w, int sum)
{
    ...
    for (int i = 0; i < (int)w.size(); i++)
    {
        for (int s = 0; s <= sum; s++)
        {
            if (rem[s] >= 0) rem[s] = cnt[i];
            else if (s < w[i] || rem[s - w[i]] <= 0) rem[s] = -1;
            else rem[s] = rem[s - w[i]] - 1;
        }
    }
    return rem[sum] >= 0;
}
