...
调用 `diameter2(h)` 时 `h` 按逆时针给出且不得重复首点，返回最远点对距离平方，$O(h)$ 时间、$O(1)$ 额外空间。`farthestPair(p)` 接受至少两个、可重复且无需预建凸包的点，返回一对最远点的原下标；全相同点返回两个不同下标。它先以 $O(n\log n)$ 建凸包，再用 $O(h)$ 旋转卡壳，额外空间 $O(n)$。静态建包使用 [11.3.001 Andrew / Graham 静态凸包](#113001)，只插点且需要在线维护才考虑 [11.3.002 单点插入动态凸包](#113002)。平移碰撞与凸多边形距离可转为 [11.3.008 闵可夫斯基和](#113008)。
