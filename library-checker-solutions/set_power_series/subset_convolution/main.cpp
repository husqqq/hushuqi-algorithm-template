...
int main() {
    ...
    cin >> n;
    int size = 1 << n;
    vector a(size, vector<Mint>(n + 1)), b = a;
    for (int mask = 0; mask < size; ++mask) {
        int value;
        cin >> value;
        a[mask][popcount(unsigned(mask))] = value;
    }
    for (int mask = 0; mask < size; ++mask) {
        int value;
        cin >> value;
        b[mask][popcount(unsigned(mask))] = value;
    }
    for (int bit = 0; bit < n; ++bit) {
        for (int mask = 0; mask < size; ++mask) {
            if (mask >> bit & 1) {
                for (int degree = 0; degree <= n; ++degree) {
                    a[mask][degree] += a[mask ^ (1 << bit)][degree];
                    b[mask][degree] += b[mask ^ (1 << bit)][degree];
                }
            }
        }
    }
    for (int mask = 0; mask < size; ++mask) {
        vector<Mint> product(n + 1);
        for (int degree = 0; degree <= n; ++degree) {
            for (int left = 0; left <= degree; ++left) product[degree] += a[mask][left] * b[mask][degree - left];
        }
        a[mask] = move(product);
    }
    for (int bit = 0; bit < n; ++bit) {
        for (int mask = 0; mask < size; ++mask) {
            if (mask >> bit & 1) {
                for (int degree = 0; degree <= n; ++degree) a[mask][degree] -= a[mask ^ (1 << bit)][degree];
            }
        }
    }
    for (int mask = 0; mask < size; ++mask) {
        cout << a[mask][popcount(unsigned(mask))].val() << " \n"[mask + 1 == size];
    }
}
