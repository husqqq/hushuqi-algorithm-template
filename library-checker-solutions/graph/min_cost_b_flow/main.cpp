...
class MinCostFlow {
  ...
  private:
    ...
    priority_queue<pair<Cost, int>, vector<pair<Cost, int>>,
                   greater<>> pq; // should be empty outside of dual()
    ...
    bool dual(const Flow delta) {
        ...
        excess_vs.erase(remove_if(begin(excess_vs), end(excess_vs), [&](const V_id v) { return b[v] < delta; }),
                        end(excess_vs));
        deficit_vs.erase(remove_if(begin(deficit_vs), end(deficit_vs), [&](const V_id v) { return b[v] > -delta; }),
                         end(deficit_vs));
    ...
    vector<Cost> get_potential() {
        ...
        for (size_t i = 0; i < n; ++i) {
            for (const auto& es : g) {
                for (const auto& e : es) {
                    if (e.residual_cap() > 0) {
                        potential[e.dst] = min(potential[e.dst], potential[e.src] + e.cost);
                    }
                }
            }
        }
...
template <class T> string i2s(T value) {
    if (value < 0) {
        return "-" + i2s(-value);
    }
    if (value == 0) {
...
int main(void) {
    ...
    for (int i__COUNTER__ = (int)(0); i__COUNTER__ < (int)(m); ++i__COUNTER__) {
        const int s = readI();
        const int t = readI();
    ...
    if (status == Status::INFEASIBLE) {
        ...
        const set<size_t> cut_set(cut_vec.begin(), cut_vec.end());
        Flow left_sum = 0, right_sum = 0, cap_sum = 0;
        for (int v = (int)(0); v < (int)(n); ++v) {
            (cut_set.count(v) ? left_sum : right_sum) += original_bs[v];
        }
        for (auto& e : edges) {
