#ifndef TEMPLATE_INLINE_D166BCD6A2DEFED2F7D1
#define TEMPLATE_INLINE_D166BCD6A2DEFED2F7D1
...
class RangeParallelUnionFind {
    class Dsu {
      ...
      public:
        explicit Dsu(int n = 0) : parent(n, -1) {}
        ...
        optional<pair<int, int>> merge(int x, int y) {
            x = find(x), y = find(y);
    ...
    template <class Callback> void mergeBlock(int level, int first, int second, Callback&& callback) {
        auto result = levels[level].merge(first, second);
        if (!result) return;
        if (!level) {
        ...
        int half = 1 << (level - 1);
        mergeBlock(level - 1, first, second, callback);
        mergeBlock(level - 1, first + half, second + half, callback);
    }

  public:
    explicit RangeParallelUnionFind(int n) {
        int levelCount = max(1, int(bit_width(unsigned(max(1, n - 1)))));
        for (int level = 0; level < levelCount; ++level) levels.emplace_back(n - (1 << level) + 1);
    }

    // Unites [first, first + length) and [second, second + length).
    // callback(root, absorbedRoot) is called for every newly united element component.
    template <class Callback> void merge(int first, int second, int length, Callback&& callback) {
        if (!length) return;
        if (length == 1) return mergeBlock(0, first, second, callback);
        int level = bit_width(unsigned(length - 1)) - 1;
        int block = 1 << level;
        mergeBlock(level, first, second, callback);
        mergeBlock(level, first + length - block, second + length - block, callback);
    }
};
#endif
...
using Mint = acl::ModInt998244353;
...
int main() {
    ...
    Mint answer = 0;
    RangeParallelUnionFind dsu(n);
    auto unite = [&](int root, int absorbed) {
        answer += value[root] * value[absorbed];
        value[root] += value[absorbed];
    };
    while (q--) {
        int length, first, second;
        cin >> length >> first >> second;
        dsu.merge(first, second, length, unite);
        cout << answer.val() << '\n';
    }
}
