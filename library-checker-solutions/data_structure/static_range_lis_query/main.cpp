...
// Problem: static_range_lis_query
#include <bits/stdc++.h>
using namespace std;
...
class bit_vector {
  ...
  public:
    ...
    int rank(const uint i) const { return v[i / w].sum - popcount(v[i / w].bit & ~uint(0) << i % w); }
...
class wavelet_matrix {
  private:
    template <class I> static bool test(const I x, const int k) { return (x & I(1) << k) != I(0); }

    vector<bit_vector> mat;

  public:
    template <class I> wavelet_matrix(const int bit_length, vector<I> a) : mat(bit_length, bit_vector(a.size())) {
        const int n = a.size();
        vector<I> a0;
        a0.reserve(n);
...
wavelet_matrix convert(const vi& p) {
    ...
    return wavelet_matrix(bit_length, move(row));
}
...
class range_lis_query {
  ...
  public:
    range_lis_query() : range_lis_query(vector<int>()) {}
