...
template <int32_t MOD> istream& operator>>(istream& in, mint<MOD>& n) {
    int64_t value;
    in >> value;
    n = value;
    return in;
}
template <int32_t MOD> ostream& operator<<(ostream& out, mint<MOD> n) {
    return out << n.value;
}
...
struct is_proth_prime_impl {
    template <int32_t PRIME, class T> static auto check(T*) -> decltype(proth_prime<PRIME>::g, true_type());
    template <int32_t PRIME, class T> static auto check(...) -> false_type;
};
template <int32_t PRIME> struct is_proth_prime : decltype(is_proth_prime_impl::check<PRIME, nullptr_t>(nullptr)) {};
/**
 * @brief Number Theoretic Transformation (NTT) for Proth primes
 * @note O(N log N)
 * @note radix-2, decimation-in-frequency, Cooley-Tukey
 * @note cache polar (~ 2x faster)
 */
template <int32_t PRIME> void ntt_inplace(vector<mint<PRIME>>& a, bool inverse) {
    const int n = a.size();
    const int log2_n = __builtin_ctz(n);
    assert(n == 1 << log2_n);
    assert(log2_n <= proth_prime<PRIME>::b);
    // prepare rotors
    vector<mint<PRIME>> ep, iep;
    while ((int)ep.size() <= log2_n) {
        ep.push_back(mint<PRIME>(proth_prime<PRIME>::g).pow(mint<PRIME>(-1).value / (1 << ep.size())));
        iep.push_back(ep.back().inv());
    }
    // divide and conquer
    vector<mint<PRIME>> b(n);
    for (int i = (1); (i) < (int)(log2_n + 1); ++(i)) {
        int w = 1 << (log2_n - i);
...
typename enable_if<is_proth_prime<PRIME>::value, vector<mint<PRIME>>>::type
ntt_convolution(const vector<mint<PRIME>>& a_, const vector<mint<PRIME>>& b_) {
    if (a_.size() <= 32 or b_.size() <= 32) {
        vector<mint<PRIME>> c(a_.size() + b_.size() - 1);
...
typename enable_if<not is_proth_prime<MOD>::value, vector<mint<MOD>>>::type
ntt_convolution(const vector<mint<MOD>>& a, const vector<mint<MOD>>& b) {
    if (a.size() <= 32 or b.size() <= 32) {
        vector<mint<MOD>> c(a.size() + b.size() - 1);
    ...
    constexpr array<int, 3> PRIMES = {1004535809, 998244353, 985661441};
    vector<mint<PRIMES[0]>> x0(a.size());
    vector<mint<PRIMES[1]>> x1(a.size());
    vector<mint<PRIMES[2]>> x2(a.size());
    for (int i = 0; (i) < (int)(a.size()); ++(i)) {
        x0[i] = a[i].value;
        x1[i] = a[i].value;
        x2[i] = a[i].value;
    }
    vector<mint<PRIMES[0]>> y0(b.size());
    vector<mint<PRIMES[1]>> y1(b.size());
    vector<mint<PRIMES[2]>> y2(b.size());
    for (int j = 0; (j) < (int)(b.size()); ++(j)) {
        y0[j] = b[j].value;
        y1[j] = b[j].value;
        y2[j] = b[j].value;
    }
    vector<mint<PRIMES[0]>> z0 = ntt_convolution<PRIMES[0]>(x0, y0);
    vector<mint<PRIMES[1]>> z1 = ntt_convolution<PRIMES[1]>(x1, y1);
    vector<mint<PRIMES[2]>> z2 = ntt_convolution<PRIMES[2]>(x2, y2);
    vector<mint<MOD>> c(z0.size());
