...
int main() {
    ...
    int n, m;
    cin >> n >> m;
    vector<long long> concave(n), arbitrary(m);
    for (auto& value : concave) cin >> value;
    for (auto& value : arbitrary) cin >> value;
    auto result = minPlusConcaveArbitrary(concave, arbitrary);
