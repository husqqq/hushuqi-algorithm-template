...
template <class Int = long long, class Int2 = long long> struct VecI2 {
    Int x, y;
    VecI2() : x(0), y(0) {}
    VecI2(pair<Int, Int> _p) : x(move(_p.first)), y(move(_p.second)) {}
    VecI2(Int _x, Int _y) : x(move(_x)), y(move(_y)) {}
...
template <class Elem> class CsrArray {
  public:
    ...
    struct ConstListRange {
        using iterator = typename vector<Elem>::const_iterator;
        iterator begi, endi;
        iterator begin() const { return begi; }
        iterator end() const { return endi; }
        int size() const { return (int)distance(begi, endi); }
  ...
  public:
    ...
    static CsrArray Construct(int n, vector<pair<int, Elem>> items) {
        CsrArray res;
        res.m_n = n;
        ...
        for (int i = (int)items.size() - 1; i >= 0; i--) {
            res.m_list[--buf[items[i].first]] = move(items[i].second);
        }
        res.m_pos = move(buf);
        return res;
    }
    static CsrArray FromRaw(vector<Elem> list, vector<int> pos) {
        CsrArray res;
        res.m_n = pos.size() - 1;
        res.m_list = move(list);
        res.m_pos = move(pos);
        return res;
    }
...
template <class Int = long long, class Int2 = long long, class Int3 = Int2> class DelaunayTriangulation {
  public:
  ...
  private:
    ...
    pair<int, int> newEdge(int u, int v) {
    ...
    pair<int, int> goNext(int, int ea) {
        int ap = edges[ea].to;
        int eap = edges[edges[ea].rev].ccw;
        return {ap, eap};
    }

    pair<int, int> goPrev(int, int ea) {
        int ap = edges[edges[ea].cw].to;
        int eap = edges[edges[ea].cw].rev;
        return {ap, eap};
    }

    tuple<int, int, int, int> goBottom(int a, int ea, int b, int eb) {
        while (true) {
    ...
    pair<int, int> getMaximum(int a, int ea, bool toMin) {
        pair<int, int> ans = {a, ea};
        int p = a, ep = ea;
        do {
            tie(p, ep) = goNext(p, ep);
            if (toMin) {
                ans = min(ans, make_pair(p, ep));
            } else {
                ans = max(ans, make_pair(p, ep));
            }
        } while (ep != ea);
        return ans;
    }
    ...
    pair<int, int> dfs(int a, int ea, int b, int eb) {
        tie(a, ea) = getMaximum(a, ea, false);
        tie(b, eb) = getMaximum(b, eb, true);
    ...
    pair<int, int> solveRange(int l, int r) {
        if (r - l == 2) {
            int u = l;
            int v = l + 1;
    ...
    void solve() {
        ...
        vector<int> pi(pos.size());
        for (int i = 0; i < (int)pi.size(); i++) {
            pi[i] = i;
        }
        stable_sort(pi.begin(), pi.end(),
                    [&](int l, int r) { return pos[l].x != pos[r].x ? pos[l].x < pos[r].x : pos[l].y < pos[r].y; });
        auto posbuf = pos;
        int posptr = 0;
  ...
  public:
    ...
    DelaunayTriangulation(vector<GPos2> x_points) : pos(move(x_points)) { solve(); }
...
int main() {
    ...
    stable_sort(tri.begin(), tri.end(),
                [&](pair<int, int> l, pair<int, int> r) -> bool { return weight(l) < weight(r); });
