...
struct TENS {
  ...
  private:
    array<long double, offset * 2 + 1> _tend{};
};
} // namespace MultiPrecisionIntegerImpl
...
struct MultiPrecisionInteger {
  ...
  private:
    ...
    static pair<vector<int>, vector<int>> _divmod_dc(const vector<int>& a, const vector<int>& b) {
        ...
        while (_lt(rem, yqh)) {
            _sub(qh, {1}), yqh = _sub(yqh, y);
        }
        rem = _sub(rem, yqh);
        while (_leq(y, rem)) {
            _add(qh, {1}), rem = _sub(rem, y);
        }
        vector<int> q, r;
        if (xu + yv == s) {
...
int main() {
    ...
    while (tests--) {
        string first, second;
        cin >> first >> second;
        auto [quotient, remainder] = divmod(bigint(first), bigint(second));
        cout << quotient.to_string() << ' ' << remainder.to_string() << '\n';
    }
    return 0;
}
