...
int main() {
    ...
    int n, m, k;
    cin >> n >> m >> k;
    BinaryMatrix left(n, m), right(m, k);
    for (int row = 0; row < n; ++row) {
        string input;
        cin >> input;
        for (int column = 0; column < m; ++column) left.set(row, column, input[column] == '1');
    }
    for (int row = 0; row < m; ++row) {
        string input;
        cin >> input;
        for (int column = 0; column < k; ++column) right.set(row, column, input[column] == '1');
    }
    BinaryMatrix result = BinaryMatrix::multiply(left, right);
    for (int row = 0; row < n; ++row) cout << result.rowString(row) << '\n';
}
