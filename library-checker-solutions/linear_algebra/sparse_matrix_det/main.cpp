...
struct Random {
  ...
  public:
    ...
    template <class T> vector<T> perm(size_t n) {
        vector<T> idx(n);
        iota(idx.begin(), idx.end(), T(0));
        shuffle(idx.begin(), idx.end());
        return idx;
    }

    template <class T> vector<T> choice(size_t n, T lower, T upper) {
        assert(n <= upper - lower + 1);
        set<T> res;
        while (res.size() < n) {
            res.insert(uniform(lower, upper));
        }
