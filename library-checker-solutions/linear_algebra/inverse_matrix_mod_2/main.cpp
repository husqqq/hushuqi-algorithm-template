...
int main() {
    ...
    int n;
    cin >> n;
    BinaryMatrix matrix(n, n * 2);
    for (int row = 0; row < n; ++row) {
        string input;
        cin >> input;
        for (int column = 0; column < n; ++column) matrix.set(row, column, input[column] == '1');
        matrix.set(row, n + row);
    }
    auto [rank, pivot] = matrix.eliminate(n);
    if (rank < n) return cout << -1 << '\n', 0;
    for (int row = 0; row < n; ++row) cout << matrix.rowString(row, n, n * 2) << '\n';
}
