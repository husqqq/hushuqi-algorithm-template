...
int main() {
    ...
    int n, k;
    cin >> n >> k;
    vector<pair<int, int>> terms(k);
    for (auto& [degree, value] : terms) cin >> degree >> value;
    vector<int> answer(n);
    long long inverseConstant = power(terms[0].second, P - 2);
    answer[0] = inverseConstant;
    for (int i = 1; i < n; i++) {
        long long value = 0;
        for (auto [degree, coefficient] : terms) {
            if (degree == 0 || degree > i) continue;
            value += 1LL * coefficient * answer[i - degree] % P;
        }
        answer[i] = (P - value % P) * inverseConstant % P;
    }
