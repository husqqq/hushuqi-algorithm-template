...
// Problem: composition_of_formal_power_series
using namespace std;
template <typename T, typename F> struct FPS_BASE : vector<T> {
    ...
    bool operator==(P x) {
        for (int i = 0; i < (int)max((*this).size(), x.size()); ++i) {
            if (i >= (int)(*this).size() && x[i] != T()) {
                return 0;
            }
            if (i >= (int)x.size() && (*this)[i] != T()) {
                return 0;
            }
    ...
    inline P pre(int sz) const { return P(begin(*this), begin(*this) + min((int)this->size(), sz)); }
    P rev(int deg = -1) {
        P ret(*this);
        if (deg != -1) {
            ret.resize(deg, T(0));
...
template <typename Mint> struct _FPS9 {
    template <typename T> T operator()(T s, T t) {
        if (s == decltype(s)()) {
            return decltype(s)();
        }
        if (t == decltype(t)()) {
...
class mint {
  ...
  public:
    ...
    constexpr static u64 get_mod() { return 998244353LL; }
};
