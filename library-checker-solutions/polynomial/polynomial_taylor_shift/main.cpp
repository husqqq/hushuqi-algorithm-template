...
int main() {
    ...
    int n, shift;
    cin >> n >> shift;
    vector<Mint> factorial(n, 1), inverseFactorial(n, 1), left(n), right(n);
    for (int i = 1; i < n; ++i) factorial[i] = factorial[i - 1] * i;
    inverseFactorial[n - 1] = factorial[n - 1].inv();
    for (int i = n - 1; i; --i) inverseFactorial[i - 1] = inverseFactorial[i] * i;
    for (int i = 0; i < n; ++i) {
        int coefficient;
        cin >> coefficient;
        left[n - 1 - i] = Mint(coefficient) * factorial[i];
    }
    Mint power = 1;
    for (int i = 0; i < n; ++i) {
        right[i] = power * inverseFactorial[i];
        power *= shift;
    }
    auto product = acl::convolution(left, right);
    for (int i = 0; i < n; ++i) {
        cout << (product[n - 1 - i] * inverseFactorial[i]).val() << " \n"[i + 1 == n];
    }
}
