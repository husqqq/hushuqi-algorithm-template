...
vector<int> batchInverse(const vector<int>& values) {
    int n = values.size();
    vector<int> prefix(n + 1, 1), inverse(n);
    for (int i = 0; i < n; i++) {
        prefix[i + 1] = 1LL * prefix[i] * (values[i] ? values[i] : 1) % P;
    }
    long long suffix = power(prefix[n], P - 2);
    for (int i = n - 1; i >= 0; i--) {
        inverse[i] = values[i] ? suffix * prefix[i] % P : 0;
        suffix = suffix * (values[i] ? values[i] : 1) % P;
    }
    return inverse;
}
...
int main() {
    ...
    for (int& x : samples) cin >> x;

    vector<int> factorial(n + 1, 1), inverseFactorial(n + 1, 1);
    for (int i = 1; i <= n; i++) factorial[i] = 1LL * factorial[i - 1] * i % P;
    inverseFactorial[n] = power(factorial[n], P - 2);
    for (int i = n; i; i--) inverseFactorial[i - 1] = 1LL * inverseFactorial[i] * i % P;

    vector<int> first(n);
    for (int i = 0; i < n; i++) {
        first[i] = 1LL * samples[i] * inverseFactorial[i] % P * inverseFactorial[n - 1 - i] % P;
        if ((n - 1 - i) & 1) first[i] = first[i] ? P - first[i] : 0;
    }
    vector<int> denominators(n + m - 1);
    for (int i = 0; i < int(denominators.size()); i++) {
        denominators[i] = (start - n + 1LL + i) % P;
        if (denominators[i] < 0) denominators[i] += P;
    }
    auto second = batchInverse(denominators);
    auto sum = convolution(first, second);

    vector<int> stepDenominator(m);
    for (int i = 0; i < m; i++) {
        int x = (start + 1LL * i) % P;
        if (x >= n) stepDenominator[i] = x - n;
    }
    auto inverseStep = batchInverse(stepDenominator);
    ...
    for (int i = 0; i < m; i++) {
        ...
        if (previous >= n && x == previous + 1) {
            falling = falling * x % P * inverseStep[i] % P;
        } else {
            falling = 1;
            for (int value = 0; value < n; value++) falling = falling * (x - value) % P;
        }
        answer[i] = falling * sum[i + n - 1] % P;
        previous = x;
    }
    printPolynomial(answer);
}
