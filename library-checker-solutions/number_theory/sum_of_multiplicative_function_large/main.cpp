...
// Problem: sum_of_multiplicative_function_large
#include <bits/stdc++.h>
using namespace std;
...
vector<int> prime_enumerate(int N) {
    ...
    for (int p = 5, d = 4, i = 1, sqn = sqrt(N); p <= sqn; p += d = 6 - d, i++) {
        ...
        for (int q = p * p / 3, r = d * p / 3 + (d * p % 3 == 2), s = 2 * p, qe = sieve.size(); q < qe;
             q += r = s - r) {
            sieve[q] = 0;
        }
    }
...
// f(p, c) : f(p^c) の値を返す
template <typename T, T (*f)(long long, long long)> struct mf_prefix_sum {
    using i64 = long long;
...
long long A, B;
mint f(long long p, long long e) {
    return A * e + B * p;
}
