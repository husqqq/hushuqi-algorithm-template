...
int main() {
    ...
    int n;
    cin >> n;
    vector<int> factorial(n + 2, 1), inverseFactorial(n + 2, 1);
    for (int i = 1; i <= n + 1; i++) factorial[i] = 1LL * factorial[i - 1] * i % P;
    inverseFactorial[n + 1] = power(factorial[n + 1], P - 2);
    for (int i = n + 1; i; i--) inverseFactorial[i - 1] = 1LL * inverseFactorial[i] * i % P;
    vector<int> series(n + 1);
    for (int i = 0; i <= n; i++) series[i] = inverseFactorial[i + 1];
    auto answer = inverseSeries(series, n + 1);
    for (int i = 0; i <= n; i++) answer[i] = 1LL * answer[i] * factorial[i] % P;
