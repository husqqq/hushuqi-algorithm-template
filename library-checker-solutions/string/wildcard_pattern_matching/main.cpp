#ifndef TEMPLATE_INLINE_0AC8893E15CF82164A14
#define TEMPLATE_INLINE_0AC8893E15CF82164A14
...
class WildcardMatcher {
  public:
    static vector<bool> match(const string& text, string pattern, char wildcard = '*') {
        int n = text.size(), m = pattern.size();
        reverse(pattern.begin(), pattern.end());
        array<vector<long long>, 3> left, right;
        for (auto& values : left) values.resize(n);
        for (auto& values : right) values.resize(m);
        for (int i = 0; i < n; ++i) fillPowers(left, i, text[i] == wildcard ? 0 : text[i] - 'a' + 1);
        for (int i = 0; i < m; ++i) fillPowers(right, i, pattern[i] == wildcard ? 0 : pattern[i] - 'a' + 1);

        vector<long long> difference(n - m + 1);
        for (int power = 0; power < 3; ++power) {
            auto product = acl::convolution_ll(left[power], right[2 - power]);
            for (int start = 0; start + m <= n; ++start) {
                long long value = product[start + m - 1];
                difference[start] += power == 1 ? -2 * value : value;
            }
        }
        vector<bool> answer(difference.size());
        for (int i = 0; i < int(answer.size()); ++i) answer[i] = difference[i] == 0;
        return answer;
    }

  private:
    static void fillPowers(array<vector<long long>, 3>& values, int index, long long value) {
        values[0][index] = value;
        values[1][index] = value * value;
        values[2][index] = value * value * value;
    }
};
#endif
...
int main() {
    ...
    string text, pattern;
    cin >> text >> pattern;
    for (bool matches : WildcardMatcher::match(text, pattern)) cout << matches;
    cout << '\n';
}
