...
template <class T> vc<vc<T>> matrix_mul(const vc<vc<T>>& A, const vc<vc<T>>& B, int N1 = -1, int N2 = -1, int N3 = -1) {
    if (N1 == -1) {
        N1 = ll(A.size()), N2 = ll(B.size()), N3 = ll(B[0].size());
    }
    vector<vector<u32>> b(N3, vector<u32>(N2));
    for (ll i = 0; i < ll(N2); ++i) {
        for (ll j = 0; j < ll(N3); ++j) {
            b[j][i] = B[i][j].val;
        }
    }
    vector<vector<T>> C(N1, vector<T>(N3));
    if ((T::get_mod() < (1 << 30)) && N2 <= 16) {
        for (ll i = 0; i < ll(N1); ++i) {
            for (ll j = 0; j < ll(N3); ++j) {
                u64 sm = 0;
                for (ll m = 0; m < ll(N2); ++m) {
                    sm += u64(A[i][m].val) * b[j][m];
                }
                C[i][j] = sm;
            }
        }
...
template <class T> struct Monoid_Mul {
    using value_type = T;
    using X = T;
...
template <class Monoid> struct Sliding_Window_Aggregation {
...
template <typename T> T lagrange_interpolate_iota(vc<T>& f, T c) {
    int n = ll(f.size());
    if (int(c.val) < n) {
        return f[c.val];
    }
...
// Complexity: M(n, n + m)
template <typename mint> vc<mint> lagrange_interpolate_iota(vc<mint>& f, mint c, int m) {
    if (m <= 60) {
        vc<mint> ANS(m);
        for (ll i = 0; i < ll(m); ++i) {
    ...
    vc<mint> b(n + m - 1);
    for (ll i = 0; i < ll(n + m - 1); ++i) {
        b[i] = mint(1) / (c + mint(i - n + 1));
    }
    a = convolution(a, b);
...
template <typename T> vc<vc<T>> prefix_product_of_poly_matrix(vc<vc<vc<T>>>& A, ll k) {
    int n = ll(A.size());
    using MAT = vc<vc<T>>;
    auto shift = [&](vc<MAT>& G, T x) -> vc<MAT> {
        int d = ll(G.size());
        vector<vector<vector<T>>> H(d, vector<vector<T>>(n, vector<T>(n)));
        for (ll i = 0; i < ll(n); ++i) {
            for (ll j = 0; j < ll(n); ++j) {
                vc<T> g(d);
                for (ll l = 0; l < ll(d); ++l) {
    ...
    T iv = T(1) / T(v);
    for (ll i = 0; i < ll(ll(G.size())); ++i) {
        T x = T(v) * T(i);
        vector<vector<T>> mat(n, vector<T>(n));
        for (ll j = 0; j < ll(n); ++j) {
            for (ll k = 0; k < ll(n); ++k) {
                mat[j][k] = evaluate(A[j][k], x);
            }
        }
        G[i] = mat;
    }
    ...
    ll i = 0;
    while (i + v <= k) {
        res = matrix_mul(G[i / v], res), i += v;
    }
    while (i < k) {
        vector<vector<T>> mat(n, vector<T>(n));
        for (ll j = 0; j < ll(n); ++j) {
            for (ll k = 0; k < ll(n); ++k) {
                mat[j][k] = evaluate(A[j][k], i);
            }
        }
...
template <typename T> T prefix_product_of_poly(vc<T>& f, ll k) {
    vc<vc<vc<T>>> A(1);
    A[0].resize(1);
    A[0][0] = f;
