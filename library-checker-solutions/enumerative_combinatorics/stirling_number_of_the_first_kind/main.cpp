...
vector<int> productRange(int l, int r) {
    if (r - l == 1) return {(P - l) % P, 1};
    int mid = (l + r) / 2;
    return convolution(productRange(l, mid), productRange(mid, r));
}
int main() {
    ...
    int n;
    cin >> n;
    vector<int> answer = n == 0 ? vector<int>{1} : productRange(0, n);
