...
int scan_int() {
    int v;
    if (::scanf("%d", &v) != 1) {
        throw runtime_error{"Fail to read."};
    }
    return v;
}
void print_int(int v) {
    if (::printf("%d\n", v) < 0) {
        throw runtime_error{"Fail to write."};
    }
}
} // namespace io
...
using polynomial = vector<mint>;

using formal_power_series = vector<mint>;

formal_power_series fps_inv(const formal_power_series& f, int n) {
    assert(f.size() and f[0] != 0);
    formal_power_series g{f[0].inv()};
    for (int k = 1; k < n; k *= 2) {
        vector<mint> f_fft(f.begin(), f.begin() + min<int>(2 * k, f.size()));
        vector<mint> g_fft(g.begin(), g.end());
...
struct factorial {
    ...
    static void ensure(int size) {
        ...
        const int next_size = max(curr_size * 2, size);
  ...
  private:
    static inline vector<mint> _fact{1}, _inv_fact{1};
};
...
vector<mint> shift_of_sampling_points(const vector<mint>& ys, mint t, int m) {
    ...
    vector<mint> b = [&] {
        ...
        vector<mint> b = atcoder::convolution(f, g);
        b.resize(n);
        return b;
    }();
    vector<mint> e = [&] {
        ...
        vector<mint> e = atcoder::convolution(b, c);
        e.resize(n);
        return e;
    }();
    ...
    vector<mint> res = atcoder::convolution(e, f);
    res.resize(m);
    for (int i = 0; i < m; ++i) {
...
struct PrecalcFactorial {
  ...
  private:
    static inline vector<mint> _fact = [] {
        vector<mint> f{1};
        f.reserve(BLOCK_SIZE);
        for (int i = 0; i < LOG_BLOCK_SIZE; ++i) {
            vector<mint> g = shift_of_sampling_points(f, 1 << i, 3 << i);
        ...
        if (BLOCK_NUM > BLOCK_SIZE) {
            vector<mint> g = shift_of_sampling_points(f, BLOCK_SIZE, BLOCK_NUM - BLOCK_SIZE);
            move(g.begin(), g.end(), back_inserter(f));
        } else {
            f.resize(BLOCK_NUM);
        }
  ...
  public:
    ...
    // (k * BLOCK_SIZE)!
    static mint kth_block_fact(int k) { return k <= BLOCK_NUM ? _fact[k] : 0; }
};

vector<mint> solve(const vector<int>& ns) {
    constexpr int block_size = PrecalcFactorial::BLOCK_SIZE;

    const int query_num = ns.size();

    int leaf_num = block_size - 1;
    vector<vector<pair<int, int>>> qs(block_size);
    ...
    vector<int> qid(query_num, -1);

    vector<polynomial> polys(2 * m), points(2 * m);
    {
        int i = 0;
    ...
    // reuse space
    vector<polynomial>& left_polys_mod_points = points;
...
int main() {
    ...
    const vector<mint> ans = solve(ns);
