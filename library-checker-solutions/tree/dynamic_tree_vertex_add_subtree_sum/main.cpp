...
// Problem: dynamic_tree_vertex_add_subtree_sum
using namespace std;
using i64 = long long;
struct cluster {
    i64 sum;
    using V = i64;
    cluster(i64 s) : sum(s) {}
    static cluster identity() { return cluster(0); }
    static cluster compress(const cluster& a, const cluster& b, V, V, V cv) { return cluster(a.sum + b.sum + cv); }
    static cluster rake(const cluster& a, const cluster& b, V, V bv, V) { return cluster(a.sum + b.sum + bv); }
    static cluster reverse(const cluster& c) { return c; }
};
class vertex;
class node;
int parent_dir(node*);
...
class vertex_raw {
  ...
  public:
    ...
    void set_value(cluster::V val) { this->val = val; }
};
class vertex {
    vertex_raw* ver;
...
class node {
  ...
  public:
    node() : par(nullptr), ra(nullptr), me(nullptr), rev(false), fo(cluster::identity()), ty(Type::None) {}
    ...
    inline void fix() {
        if (this->ty == Type::Edge) {
            ...
            if (this->rake()) {
                node* r = this->rake();
                left = cluster::rake(l->fold(), r->fold(), l->endpoint(0).value(), r->endpoint(0).value(),
                                     l->endpoint(1).value());
            }
            node* r = this->child(1);
            this->fo = cluster::compress(left, r->fold(), l->endpoint(0).value(), r->endpoint(1).value(),
                                         l->endpoint(1).value());
            ...
            this->fo =
                cluster::rake(this->child(0)->fold(), this->child(1)->fold(), this->child(0)->endpoint(0).value(),
                              this->child(1)->endpoint(0).value(), this->child(0)->endpoint(1).value());
        } else {
            assert(false);
        }
    }
    ...
    inline node* child(size_t dir) const { return this->ch[dir]; }
    inline void set_child(node* ch, size_t dir) { this->ch[dir] = ch; }
    inline vertex endpoint(size_t dir) { return this->v[dir]; }
    ...
    while (true) {
        ...
        node* n = nullptr;
        {
            ...
            if (!par) {
                ...
                par->push();
                if (par->guard && parent_dir_guard(t) != -1) {
                    break;
                }
                n = par;
            } else {
...
int main() {
    ...
    for (int i = 0; i < Q; i++) {
        ...
        if (c == 0) {
            ...
            cut(vs[a], vs[b]);
            expose(vs[a]);
            cout << vs[a].handle()->fold().sum + vs[a].handle()->endpoint(0).value() +
                        vs[a].handle()->endpoint(1).value()
                 << '\n';
            link(vs[a], vs[b], cluster::identity());
        }
    }
