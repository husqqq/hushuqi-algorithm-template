...
// Problem: dynamic_tree_vertex_set_path_composite
using namespace std;
template <class Monoid> struct reversible_monoid {
    using base_type = typename Monoid::value_type;
    using value_type = pair<base_type, base_type>;
    Monoid base;
    reversible_monoid() = default;
    reversible_monoid(const Monoid& base_) : base(base_) {}
    value_type unit() const { return make_pair(base.unit(), base.unit()); }
    value_type mult(const value_type& a, const value_type& b) const {
        return make_pair(base.mult(a.first, b.first), base.mult(b.second, a.second));
    }
    static value_type make(const base_type& x) { return make_pair(x, x); }
    static value_type reverse(const value_type& a) { return make_pair(a.second, a.first); }
 ...
 * @note in each splay tree, nodes are sorted from bottom to top. the rightmost node of the root splay tree of the
 * auxiliary tree is the root of represented tree.
 */
template <class Monoid> class link_cut_tree {
    using value_type = typename Monoid::value_type;
    using reversible_type = typename reversible_monoid<Monoid>::value_type;
    const reversible_monoid<Monoid> mon;
    vector<value_type> data;         // data of the original tree
    vector<reversible_type> path;    // sum of data of the sub-tree in the belonging splay tree
    vector<int> parent, left, right; // of the auxiliary tree
    vector<bool> is_reversed;        // of the auxiliary tree; not applied to left, right, and path yet
    /**
    ...
    void propagate_reverse_node(int a) {
        if (is_reversed[a]) {
            ...
            if (right[a] != -1) {
                is_reversed[right[a]] = not is_reversed[right[a]];
            }
            if (left[a] != -1) {
                is_reversed[left[a]] = not is_reversed[left[a]];
            }
            swap(left[a], right[a]);
        }
    }
    void propagate_reverse_splay(int a) {
        ...
        if (left[a] != -1) {
            propagate_reverse_node(left[a]);
        }
        if (right[a] != -1) {
            propagate_reverse_node(right[a]);
        }
    }
    /**
    ...
    void update_path(int a) {
        ...
        if (right[a] != -1) {
            path[a] = mon.mult(path[a], path[right[a]]);
        }
        if (left[a] != -1) {
            path[a] = mon.mult(path[left[a]], path[a]);
        }
    }

  public:
    link_cut_tree(int size, const Monoid& mon_ = Monoid())
        : mon(mon_), data(size, mon.base.unit()), path(size, mon.unit()), parent(size, -1), left(size, -1),
          right(size, -1), is_reversed(size, false) {}
    /**
    ...
    int get_lowest_common_ancestor(int a, int b) {
        ...
        assert(not is_reversed[a]);
        int preserved = -1;
        swap(left[a], preserved); // make `a` and `b` belong different splay trees even if `b` is a descendant of `a`
        int result = b;
        int c = b;
        ...
        swap(left[a], preserved);
        return c == a ? result : -1;
    }
    bool are_connected(int a, int b) { return get_lowest_common_ancestor(a, b) != -1; }
    ...
    value_type vertex_get(int a) const { return data[a]; }
        ...
        for (int c = a; c != b and c != -1; c = parent[c]) {
            ...
            switch (get_parent_edge_type(c)) {
                case -1: // heavy (left-child)
                    ...
                    if (right[parent[c]] != -1) {
                        up = mon.mult(up, path[right[parent[c]]]);
                    }
                    break;
                case 1: // heavy (right-child)
                    down = mon.mult(data_parent_c, down);
                    if (left[parent[c]] != -1) {
                        down = mon.mult(path[left[parent[c]]], down);
                    }
                    break;
                case 0: // light
...
template <class CommutativeRing> struct linear_function_monoid {
    using value_type = pair<CommutativeRing, CommutativeRing>;
 ...
 */
template <class Monoid> struct dual_monoid {
    using value_type = typename Monoid::value_type;
    Monoid base;
...
int main() {
    ...
    while (q--) {
        ...
        if (t == 0) {
            ...
            int u;
            mint<MOD> c, d;
            scanf("%d%d%d", &u, &c.value, &d.value);
            lct.vertex_set(u, make_pair(c, d));
        } else if (t == 2) {
            int u, v;
            mint<MOD> x;
            scanf("%d%d%d", &u, &v, &x.value);
