...
// Problem: dynamic_tree_vertex_add_path_sum
using namespace std;
using i64 = long long;
struct cluster {
    i64 length;
    ...
    static cluster compress(const cluster& a, const cluster& b, V, V, V cv) {
        return cluster(a.length + b.length + cv);
    }
    static cluster rake(const cluster& a, const cluster&, V, V, V) { return cluster(a.length); }
    static cluster reverse(const cluster& c) { return c; }
    static size_t select(const cluster&, const cluster&, V, V, V) { return 0; }
};
class vertex;
class node;
int parent_dir(node*);
...
int main() {
    ...
    for (int i = 0; i < Q; i++) {
        ...
        if (c == 0) {
            ...
            cout << path_query(vs[a], vs[b]).length + vs[a].value() + vs[b].value() << '\n';
        }
    }
}
