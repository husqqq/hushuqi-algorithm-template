...
// Problem: point_set_tree_path_composite_sum_fixed_root
using namespace std;
 ...
 */
template <typename T> vector<vector<T>> rooted_tree(const vector<vector<T>>& g, int root = 0) {
    int N = (int)g.size();
...
template <typename G> struct HeavyLightDecomposition {
  ...
  public:
    ...
    HeavyLightDecomposition(G& _g, int _root = 0)
        : g(_g), root(_root), id(0), size(g.size(), 0), depth(g.size(), 0), down(g.size(), -1), up(g.size(), -1),
          nxt(g.size(), root), par(g.size(), root) {
        dfs_sz(root);
        dfs_hld(root);
    }
    ...
    template <typename F> void path_noncommutative_query(int u, int v, bool vertex, const F& f) {
        int l = lca(u, v);
        for (auto&& [a, b] : ascend(u, l)) f(a + 1, b);
        if (vertex) f(down[l], down[l] + 1);
        for (auto&& [a, b] : descend(l, v)) f(a, b + 1);
    }
    template <typename F> void subtree_query(int u, bool vertex, const F& f) { f(down[u] + int(!vertex), up[u]); }
    int lca(int a, int b) {
        while (nxt[a] != nxt[b]) {
...
template <typename G> struct StaticTopTreeVertexBased {
...
template <typename G, typename Path, typename Point, typename Vertex, typename Compress, typename Rake,
          typename Add_edge, typename Add_vertex>
struct DPonStaticTopTreeVertexBased {
    ...
    DPonStaticTopTreeVertexBased(const HeavyLightDecomposition<G>& hld, const Vertex& _vertex,
                                 const Compress& _compress, const Rake& _rake, const Add_edge& _add_edge,
                                 const Add_vertex& _add_vertex)
        : tt(hld), vertex(_vertex), compress(_compress), rake(_rake), add_edge(_add_edge), add_vertex(_add_vertex) {
        int n = tt.P.size();
...
vector<mint> calc(int N, int Q, vector<int> A, vector<int> U, vector<int> V, vector<int> B, vector<int> C,
                  vector<int> cmd, vector<int> I, vector<int> X, vector<int> Y) {
    ...
    auto rake = [&](const Point& a, const Point& b) -> Point { return {a.s + b.s, a.x + b.x}; };
    ...
    DPonStaticTopTreeVertexBased<vector<vector<int>>, Path, Point, decltype(vertex), decltype(compress), decltype(rake),
...
vector<mint> naive(int N, int Q, vector<int> A, vector<int> U, vector<int> V, vector<int> B, vector<int> C,
                   vector<int> cmd, vector<int> I, vector<int> X, vector<int> Y) {
    vector<mint> ans(Q);
    for (int q = 0; q < (Q); q++) {
        if (cmd[q] == 0) {
