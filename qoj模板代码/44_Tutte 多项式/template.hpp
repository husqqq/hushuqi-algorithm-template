...
{
    // base 是任意整数，exponent 是非负指数；返回 base^exponent mod MOD。
    long long result = 1;
    while (exponent)
    {
...
{
    // a 按子集和次数分层存储；variables 是点数，inverse 决定正变换或逆变换。
...
{
    // input 是按子集存储的集合幂级数；返回其指数，inverse 是 1..variables 的模逆。
    int count = 1 << variables;
    int width = variables + 1;
...
int tutteEval(const vector<unsigned> &adjacency, int x, int y)
{
    // adjacency 是 n<=21 个顶点的无向邻接位集，x、y 是多项式点值；返回 Tutte 多项式点值。
    int n = adjacency.size();
    assert(0 < n && n <= 21 && x >= 0 && x < MOD && y >= 0 && y < MOD);
    ...
    return setExponential(connected, n, inverse).back();
}
