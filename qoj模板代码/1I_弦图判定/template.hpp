...
{
    bool ok; // 是否为弦图。
    vector<int> peo; // 弦图时的一组完美消除序列。
    vector<int> cycle; // 非弦图时的诱导环，首尾不重复。
};

ChordalCert chordalCert(int n, const vector<pair<int, int>> &edges)
{
    // n 是点数，edges 是简单无向边；返回一组 PEO，或一条长度至少四的诱导环。
    assert(n >= 0);
    vector<int> degree(n);
    for (auto [u, v] : edges)
    {
