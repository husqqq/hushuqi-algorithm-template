...
vector<Dist> eppsteinWalks(
    int n, const vector<EppsteinEdge> &e, int s, int t,
    unsigned long long k, bool includeEmpty = true)
{
    ...
    bool skipEmpty = s == t && !includeEmpty;
    vector<Dist> ans;
    ans.reserve(k);
    while (!pq.empty())
    {
...
template<class Dist = long long>
optional<Dist> eppsteinKthWalk(
    int n, const vector<EppsteinEdge> &e, int s, int t,
    unsigned long long k, bool includeEmpty = true)
{
    // 参数含义同 eppsteinWalks；返回第 k 小游走长度，不足 k 条时返回空。
    auto ans = eppsteinWalks<Dist>(n, e, s, t, k, includeEmpty);
    if (ans.size() < k)
    {
        return nullopt;
    }
