...
vector<Z> subsetConvolution(const vector<Z> &a, const vector<Z> &b)
{
...
vector<Z> transposedSubsetConvolution(vector<Z> s, vector<Z> x)
{
    // s、x 是同一位数的子集函数；返回映射 a->a*s 对 x 的转置作用结果。
    reverse(x.begin(), x.end());
    x = subsetConvolution(x, s);
    reverse(x.begin(), x.end());
    return x;
}

vector<Z> powerProjectionEgf(vector<Z> wt, const vector<Z> &s, int n)
{
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        for (int j = 0; j < n - i; j++)
        {
            ...
            b = transposedSubsetConvolution(move(a), move(b));
...
vector<Z> chromaticPoly(const vector<unsigned long long> &g)
{
    ...
    Z c = f[0];
    f[0] -= c;
    vector<Z> egf = powerProjectionEgf(wt, f, n);
