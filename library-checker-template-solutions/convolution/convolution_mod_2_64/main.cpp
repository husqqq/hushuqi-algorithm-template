                                   ...
                                   const vector<unsigned long long> &b)
{
    ...
    constexpr array<uint32_t, 5> mod{998244353, 985661441, 943718401, 935329793, 918552577};
    constexpr array<uint32_t, 5> root{3, 3, 7, 3, 5};
    array<vector<uint32_t>, 5> rem;
    for (int k = 0; k < 5; k++)
    {
        rem[k] = arbitrary_mod_convolution::NTT{mod[k], root[k]}.conv(a, b);
    }
    vector<unsigned long long> ans(need);
    for (int i = 0; i < need; i++)
    {
        array<uint32_t, 5> coef{}; // coef[k] 是第 k 个 Garner 系数。
        unsigned long long cur = 0, mul64 = 1;
        for (int k = 0; k < 5; k++)
        {
            uint64_t seen = 0, mul = 1;
            for (int j = 0; j < k; j++)
            {
                seen = (seen + mul * coef[j]) % mod[k];
                mul = mul * mod[j] % mod[k];
            }
            uint32_t delta = (rem[k][i] + mod[k] - seen % mod[k]) % mod[k];
            coef[k] = (uint64_t)delta * arbitrary_mod_convolution::inv((uint32_t)mul, mod[k]) % mod[k];
            cur += mul64 * coef[k];
            mul64 *= mod[k];
        }
        ans[i] = cur;
