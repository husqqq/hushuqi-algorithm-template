...
template <class K, class V, uint32_t N, class H = CustomHash> class FixedHashMap
{
    static_assert(1 <= N && N <= 30);
    static constexpr uint32_t CAP = 1U << N;
    ...
    uint32_t findPos(const K &x) const
    {
...
template <class K, uint32_t N, class H = CustomHash> class FixedHashSet
{
    static_assert(1 <= N && N <= 30);
    static constexpr uint32_t CAP = 1U << N;
    static constexpr uint32_t MASK = CAP - 1;
