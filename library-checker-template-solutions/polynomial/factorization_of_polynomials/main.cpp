...
vector<pair<Poly, int>> distinctDegree(Poly f)
{
...
void splitDistinctRoots(Poly f, mt19937_64 &rng, vector<Z> &ans)
{
    ...
    while (true)
    {
        ...
        Poly q = polyDivmod(move(f), g).first;
        splitDistinctRoots(move(g), rng, ans);
        splitDistinctRoots(move(q), rng, ans);
        return;
    }
}

vector<Z> distinctRoots(Poly f, unsigned long long seed = 0)
{
    ...
    mt19937_64 rng(seed);
    vector<Z> ans;
    splitDistinctRoots(polyMonic(move(g)), rng, ans);
...
vector<DynFactor> factorPolyPrime(vector<int> f, int p,
...
signed main()
{
    ...
    auto ans = factorPolyPrime(move(f), p);
    cout << ans.size() << '\n';
