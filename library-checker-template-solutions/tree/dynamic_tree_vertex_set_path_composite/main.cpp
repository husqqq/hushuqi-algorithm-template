...
template <long long P> class DynamicPathComposite
{
public:
    struct Func
    {
...
private:
    ...
    void makeRoot(int x)
    {
        // x 是内部下标；把 represented tree 的根换到 x。
        access(x);
        reverseNode(x);
    }
    int findRoot(int x)
    {
        // x 是内部下标；返回所在 represented tree 的根。
        access(x);
        push(x);
...
public:
    explicit DynamicPathComposite(const vector<Func> &f = {}) : t(f.size() + 1)
    {
        // f[u] 是点 u 的初始函数。
